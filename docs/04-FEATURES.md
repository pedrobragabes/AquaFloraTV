...
- [x] Preview inline antes de adicionar à playlist
...
- [x] Tempo de exibição por imagem (default 10s, configurável)
- [x] Remover item da playlist
- [x] Deletar playlist inteira
- [x] Duplicar item na playlist
...
- [x] Botão "Sincronizar agora" no dashboard → push SSE → sync imediato
...
- [x] Dashboard: página "Releases" pra subir nova APK
- [x] API calcula MD5 e serve download da APK enviada
...
- [x] Rollback manual: Pedro volta uma release anterior como "latest" se nova bugar
...
- [x] Alertas de armazenamento em 70% e 85% (server e device)
- [x] Limpeza automática de mídia sem uso (janela configurável 30-60 dias)
- [x] Log de eventos (boot, sync, erro, crash)
